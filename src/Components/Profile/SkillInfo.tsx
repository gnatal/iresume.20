...
import { ISkillProps } from "../../utils/DataTypes";
import {
  AddInfoButton,
  CancelEditButton,
  EditButton,
...
const SkillInfo: React.FC<ISkillProps> = ({
  SkillInfoArray,
  Edit,
  navigation,
}) => {
  ...
  if (isLoading) {
    ...
    if (SkillInfoArray != undefined && SkillInfoArray.length > 0) {
      SkillInfoArray.forEach((value, index, array) => {
        infoToRender.push(
          <View key={index}>
